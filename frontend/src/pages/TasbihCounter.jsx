...
export default function TasbihCounter() {
  ...
  setCounters(cs => cs.map(c => c._id === updated._id ? updated : c))
  const removeFromList = id =>
  setCounters(cs => cs.filter(c => c._id !== id))

  const handleIncrement = async id => { const u = await incrementCounter(id); updateList(u) }
  const handleDecrement = async id => { const u = await decrementCounter(id); updateList(u) }
  const handleReset     = async id => { const u = await resetCounter(id); updateList(u) }
  const handleDelete    = async id => { if (confirm('Delete this counter?')) { await deleteCounter(id); removeFromList(id) } }
  ...
  const saveEdit = async id => {
    try {
      const u = await updateCounter(id, editName, editTarget)
      updateList(u)
      setEditingId(null)
    } catch (e) {
      alert(e.message)
  ...
  if (loading)
    ...
    if (error)
      ...
      return (
        <>
        <NavBar />
        <div className="min-h-screen w-full relative flex items-center justify-center bg-cover bg-center pt-20" style={{ backgroundImage: "url('/home-back.jpg')", fontFamily: "'Poppins', sans-serif" }}>
        ...
        />
        <button type="submit" className="px-6 py-2 bg-yellow-500 hover:bg-yellow-400 text-gray-900 rounded-xl font-medium shadow-lg transition">
        Add
        </button>
        </form>
        ...
        {counters.map(c => (
          ...
          {editingId === c._id ? (
            ...
            <svg className="w-6 h-6 text-yellow-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M18 15l-6-6-6 6" /></svg>
            </button>
            ...
            <svg className="w-6 h-6 text-yellow-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 9l6 6 6-6" /></svg>
            </button>
            </div>
            <div className="flex justify-between items-center">
